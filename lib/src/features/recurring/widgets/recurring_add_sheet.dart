...
void showRecurringAddSheet(
  ...
  showAppBottomSheet<void>(
    context: context,
    builder: (context) => _RecurringAddSheet(kind: kind),
  );
}

class _RecurringAddSheet extends ConsumerStatefulWidget {
  const _RecurringAddSheet({required this.kind});
...
class _RecurringAddSheetState extends ConsumerState<_RecurringAddSheet> {
    ...
    return Padding(
      ...
      child: AppSheetContainer(
        child: Column(
          children: [
            AppSheetHeader(
              title: widget.kind == RecurringItemKind.transaction
  ...
  Future<void> _save() async {
    ...
    final item = RecurringItem(
      id: generateLocalEntityId(),
      kind: widget.kind,
      type: _type,
      amount: amount,
      categoryId: _categoryId!,
      sourceId: _sourceId!,
      startDate: normalizeRecurringDate(_startDate),
      frequency: _frequency,
      nextRunAt: normalizeRecurringDate(_startDate),
    ...
    if (mounted) {
      ...
      AppToast.show(
        context,
        message:
            'Đã thêm ${widget.kind == RecurringItemKind.transaction ? 'giao dịch' : 'nhắc nhở'} định kỳ',
